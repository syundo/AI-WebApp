...
--  매니져가 없는 직원 포함
select (e.first_name||' '||e.last_name) as fullname, (e2.first_name||' '||e2.last_name) as manager 
...
-- 직원이 없는 부서를 포함하는 경우
select (e.first_name||' '||e.last_name) as fullname, d.department_name, j.job_title 
from employees e 
full outer join departments d on d.department_id = e.department_id 
full outer join jobs j on j.job_id = e.job_id;
-- 직원이 없는 부서를 포함하지 않는 경우
select (e.first_name||' '||e.last_name) as fullname, d.department_name, j.job_title 
from employees e 
