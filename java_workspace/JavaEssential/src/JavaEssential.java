import java.util.ArrayList;
import java.util.HashMap;
import java.util.Map;

import com.naver.service.Person;
import com.naver.service.Person2;
import com.naver.service.Student;
import com.naver.service.Student2;

public class JavaEssential {
	/**
	 * @param args
	 */
	public static void main(String[] args) {
		Person p=new Person();
		p.name="홍길동"; // public이므로 접근 가능
		System.out.print(p.name);
		System.out.print(": ");
		p.sleep(); // public이므로 접근 가능
		
		Person2 p2=new Person2(); 
//		p2.name="이순신"; // private라서 접근 불가능
		p2.setName("이순신"); // setter 사용. 저장.
		System.out.print(p2.getName()); // getter 사용. 읽기.
		System.out.print(": ");
		p2.sleep(); // public이므로 접근 가능
		
		Student s=new Student();
		s.name="왕건";
		s.department="철학과";
		System.out.print(s.name);
		System.out.print(": ");
		s.sleep();
//		s.study(); // default 접근제한자이므로 접근 불가
		
		Person p3=new Person("을지문덕");
		System.out.println(p3.name);
		
		Student s2=new Student();
		System.out.print(s2.name);
		System.out.print("는 ");
		s2=new Student("강감찬","컴퓨터공학과");
		System.out.println(s2.department+" "+s2.name);
		
//		Person3 p4=new Person3(); // interface는 instance 생성 불가
		Student2 s3=new Student2();
		s3.name="신사임당";
		s3.department="전자공학과";
		System.out.println(s3.department+" "+s3.name);
		
		//ArrayList에 Student 객체 사용하기
		ArrayList<Student> list=new ArrayList<Student>();
		list.add(new Student());
		list.add(new Student());
		Student s4=new Student();
		list.add(s4);
		list.get(0).name="이율곡";
		list.get(0).department="국문학과";
		Student s5=list.get(1);
		s5.name="광개토대왕";
		s5.department="정치학과";
		
		//HashMap
		Map<String,Student2> map=new HashMap<>();//upcasting
		map.put("천재", new Student2());
		map.put("영재", new Student2());
		map.get("천재").name="김천재";
		map.get("천재").department="수학과";
		
		//Map에 ArrayList 넣기
		Map<String,ArrayList<Student>> map2=new HashMap<>();
		ArrayList<Student> list2=new ArrayList<>();
		list2.add(new Student("김수학","수학과"));
		list2.add(new Student("최물리","물리학과"));
		list2.add(new Student("이화학","화학과"));
		map2.put("자연과학대학", list2);
		for(Student stemp :map2.get("자연과학대학")) {
			System.out.println(stemp.department+" "+stemp.name);
		}
		
		Map<String,ArrayList<Student>> map3=new HashMap<>();
		ArrayList<Student> list3=new ArrayList<>();
		list3.add(new Student("윤컴공","컴퓨터공학과"));
		list3.add(new Student("박화공","화학공학과"));
		map3.put("공과대학", list3);
		
		//ArrayList에 Map 넣기
		ArrayList<Map<String,ArrayList<Student>>> list4=new ArrayList<>();
		list4.add(map2);
		list4.add(map3);
		for (Map<String, ArrayList<Student>> mapTemp : list4) {
		    for (Map.Entry<String, ArrayList<Student>> entry : mapTemp.entrySet()) {
		        String faculty = entry.getKey(); //key
		        ArrayList<Student> studentList = entry.getValue(); // value

		        System.out.println("[" + faculty + "]"); // key 출력. 단과대명
		        for (Student stemp : studentList) {
		            System.out.println("이름: " + stemp.name + ", 학과: " + stemp.department);
		        }
		        System.out.println(); // 줄바꿈
		    }
		}
	}
}