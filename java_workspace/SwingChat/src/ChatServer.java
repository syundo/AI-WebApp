...
public class ChatServer extends JFrame implements ActionListener {
	...
	public ChatServer() {
		...
		serverDialog=new IDDialog(this,"아이디 설정");
		serverDialog.okButton.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
            	serverDialog.id = serverDialog.idField.getText();  // 입력된 아이디 가져오기
                System.out.println("입력한 아이디: " + serverDialog.id);  // 아이디 출력 (여기서 다른 처리 가능)
	...
	private void setupConnection() throws IOException {
		listener = new ServerSocket(8080); // 서버 소켓 생성
	...
	public void actionPerformed(ActionEvent e) { // JTextField에 <Enter> 키 처리
		if (e.getSource() == sender) {
			...
			String msg = serverDialog.id+": "+msgcontent+"\n";
			try {
				out.write(msg); // 문자열 전송
				out.flush();
				
				receiver.append("\n"+serverDialog.id+": " + msgcontent);// JTextArea에 출력
				int pos = receiver.getText().length();
				receiver.setCaretPosition(pos); // caret 포지션을 가장 마지막으로 이동
